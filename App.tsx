...
export default function App() {
    ...
    const handleAvatarChange = useCallback((groupName: string, url: string) => {
        setFormData(prev => {
            ...
            if (url.trim()) {
                if (existingIndex > -1) {
                    specific[existingIndex] = { ...specific[existingIndex], url };
                } else {
                    specific.push({ itemName: groupName, url });
                }
            } else {
                if (existingIndex > -1) {
                    specific.splice(existingIndex, 1);
                }
            }
    ...
    const groupedItems = useMemo((): GroupedItem[] => {
        return groupFormItems(formData.selectedItems);
    }, [formData.selectedItems]);
    ...
    const generateWindowCfg = (data: FormData, variant: 'A' | 'B' = 'A') => {
        ...
        finalData.avatarImages.specific.forEach(spec => {
            if (spec.itemName && spec.url) {
                stepIcons[spec.itemName] = spec.url;
            }
        });
        ...
        <div className="bg-gray-50 min-h-screen p-4 sm:p-8">
            <div className="max-w-4xl mx-auto space-y-6">
                ...
                <Card>
                    ...
                    <div className="space-y-6">
                        ...
                        <div id="field-container-gtmId" className="p-4 border rounded-lg bg-gray-100">
                            ...
                            {formData.useAnalytics && <div className="space-y-4 mt-4 pl-6 border-l-2 border-teal-100">
                       ...
                       <div>
                        ...
                        <div>
                            <SectionTitle>選択した項目（順序変更可）</SectionTitle>
                             <div className="p-2 border rounded-lg min-h-[24rem] bg-white">
                                {formData.selectedItems.length > 0 ? (<ReorderableList items={formData.selectedItems} onMove={(oldIndex, newIndex) => handleMoveItem('selectedItems', oldIndex, newIndex)} />) : (<div className="text-center text-gray-500 py-10">項目を選択してください</div>)}
                            </div>
                        </div>
                    </div>
                ...
                <div className="flex justify-center pt-4">
                    <Button onClick={handleSubmit} className="w-full sm:w-auto text-lg py-3 px-8">HTMLを生成する</Button>
                </div>
            </div>
        </div>
    );
